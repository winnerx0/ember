...
const FEATURES = [
  {
    ...
    description:
      "Drag, drop, and connect tables on an infinite canvas. Design your entire schema visually with React Flow.",
    color: "#f97316",
  },
  {
    icon: "🔗",
    ...
    description:
      "Draw FK relationships between columns. Supports one-to-one, one-to-many, and many-to-many with cardinality labels.",
    color: "#3b82f6",
  },
  {
    icon: "⬇",
  ...
  },
  {
    ...
    description:
      "Every change is saved to PostgreSQL instantly. Redis caching keeps the canvas blazing fast.",
    color: "#8b5cf6",
  },
  {
    icon: "🎨",
    ...
    description:
      "Full PostgreSQL type support: UUID, JSONB, TIMESTAMPTZ, arrays, and more. Set PK, unique, nullable, defaults.",
    color: "#f59e0b",
  },
  {
    icon: "📐",
    title: "Auto Layout",
    description:
      "One-click auto-layout arranges your tables intelligently. Zoom, pan, and minimap for large schemas.",
    color: "#ec4899",
  },
];
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (dragging !== null) {
      ...
      setTablePositions((prev) =>
        prev.map((pos, i) =>
          i === dragging ? { x: Math.max(0, Math.min(newX, container.width - 160)), y: Math.max(0, Math.min(newY, container.height - 150)) } : pos
        )
      );
    }
  };
        ...
        >
          ...
          {table.columns.map((col) => (
            ...
            >
              ...
              <span className="flex-1" style={{ color: "var(--foreground)" }}>{col.name}</span>
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b backdrop-blur-xl" style={{ borderColor: "var(--border)", background: "var(--background)/80" }}>
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center" style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>
            <span className="font-black text-xs sm:text-sm">E</span>
          </div>
          <span className="font-bold text-base sm:text-lg tracking-tight">Ember</span>
          <span className="ml-1 sm:ml-2 px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-semibold border" style={{ background: "var(--accent)", color: "var(--accent-foreground)", borderColor: "var(--border)" }}>
            BETA
          </span>
        </div>
        <div className="hidden md:flex items-center gap-6 lg:gap-8 text-sm" style={{ color: "var(--muted-foreground)" }}>
      ...
      <section className="relative pt-24 sm:pt-28 md:pt-32 pb-12 sm:pb-16 md:pb-20 px-4 sm:px-6 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-medium mb-4 sm:mb-6 border" style={{ background: "var(--accent)", color: "var(--accent-foreground)", borderColor: "var(--border)" }}>
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-tight mb-4 sm:mb-6">
              Design your <span style={{ color: "var(--primary)" }}>database</span>
              <br />
              visually.
            </h1>

            <p className="text-base sm:text-lg leading-relaxed mb-6 sm:mb-8 max-w-lg" style={{ color: "var(--muted-foreground)" }}>
              Ember is a visual PostgreSQL schema designer. Create tables,
              define relationships, and export production-ready SQL — all in a
            ...
            {/* Stats */}
            <div className="flex gap-6 sm:gap-8 mt-8 sm:mt-12 pt-6 sm:pt-8 border-t" style={{ borderColor: "var(--border)" }}>
              {[
                ...
                <div key={s.label}>
                  <div className="text-xl sm:text-2xl font-black" style={{ color: "var(--primary)" }}>
                    {s.val}
                  </div>
                  <div className="text-[10px] sm:text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            ...
            >
              {/* Toolbar mock */}
              <div className="flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-3 border-b" style={{ borderColor: "var(--border)" }}>
                ...
                <span className="ml-2 sm:ml-3 text-[10px] sm:text-xs font-mono" style={{ color: "var(--muted-foreground)" }}>
                  ember · blog_schema
                </span>
              </div>
      ...
      <section id="features" className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl font-black mb-3 sm:mb-4">
            ...
            <span style={{ color: "var(--primary)" }}>ship faster</span>
          </h2>
          <p className="text-base sm:text-lg max-w-2xl mx-auto" style={{ color: "var(--muted-foreground)" }}>
            Stop writing DDL by hand. Design visually, iterate quickly, export
            instantly.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {FEATURES.map((f) => (
            ...
            >
              ...
              <h3 className="font-bold text-base sm:text-lg mb-2" style={{ color: "var(--card-foreground)" }}>{f.title}</h3>
      ...
      >
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl font-black mb-3 sm:mb-4">
              From idea to SQL in <span style={{ color: "var(--primary)" }}>minutes</span>
            </h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
            {STEPS.map((s, i) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-bold text-sm sm:text-base mb-1" style={{ color: "var(--card-foreground)" }}>{s.title}</h3>
                  <p className="text-xs sm:text-sm leading-relaxed" style={{ color: "var(--muted-foreground)" }}>
                    {s.desc}
                  </p>
                </div>
      ...
      <section className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          <div>
            ...
            <p className="text-sm sm:text-base leading-relaxed mb-4 sm:mb-6" style={{ color: "var(--muted-foreground)" }}>
              Ember generates complete PostgreSQL DDL — CREATE TABLE statements,
              foreign key constraints, indexes, and more. Copy to clipboard or
              download as a <code style={{ color: "var(--primary)" }}>.sql</code> file.
            </p>
          ...
          >
            <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3 border-b" style={{ borderColor: "var(--border)" }}>
              <span className="text-[10px] sm:text-xs font-mono" style={{ color: "var(--muted-foreground)" }}>
                schema.sql
              </span>
              <span className="text-[10px] sm:text-xs font-semibold" style={{ color: "var(--primary)" }}>
                Export ready
              </span>
            </div>
            <pre className="p-3 sm:p-5 text-[10px] sm:text-xs font-mono leading-relaxed overflow-x-auto" style={{ color: "var(--foreground)" }}>
              <code>
                ...
                <span style={{ color: "var(--primary)" }}>{"CREATE TABLE "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"users" (\n'}</span>
                <span style={{ color: "var(--foreground)" }}>{"  "}</span>
                <span style={{ color: "var(--foreground)" }}>{"  "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"id"'}</span>
                <span style={{ color: "var(--chart-1)" }}>{" UUID"}</span>
                <span style={{ color: "var(--primary)" }}>{" PRIMARY KEY"}</span>
                <span style={{ color: "var(--foreground)" }}>{",\n"}</span>
                <span style={{ color: "var(--foreground)" }}>{"  "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"email"'}</span>
                <span style={{ color: "var(--chart-1)" }}>{" VARCHAR(255)"}</span>
                <span style={{ color: "var(--primary)" }}>{" NOT NULL UNIQUE"}</span>
                <span style={{ color: "var(--foreground)" }}>{",\n"}</span>
                <span style={{ color: "var(--foreground)" }}>{"  "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"created_at"'}</span>
                <span style={{ color: "var(--chart-1)" }}>{" TIMESTAMPTZ"}</span>
                <span style={{ color: "var(--foreground)" }}>{"\n);\n\n"}</span>
                <span style={{ color: "var(--primary)" }}>{"CREATE TABLE "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"posts" (\n'}</span>
                <span style={{ color: "var(--foreground)" }}>{"  "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"id"'}</span>
                <span style={{ color: "var(--chart-1)" }}>{" UUID"}</span>
                <span style={{ color: "var(--primary)" }}>{" PRIMARY KEY"}</span>
                <span style={{ color: "var(--foreground)" }}>{",\n"}</span>
                <span style={{ color: "var(--foreground)" }}>{"  "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"user_id"'}</span>
                <span style={{ color: "var(--chart-1)" }}>{" UUID"}</span>
                ...
                <span style={{ color: "var(--primary)" }}>{"ALTER TABLE "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"posts" '}</span>
                <span style={{ color: "var(--primary)" }}>{"ADD CONSTRAINT "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"fk_posts_users"\n'}</span>
                <span style={{ color: "var(--primary)" }}>{"  FOREIGN KEY "}</span>
                <span style={{ color: "var(--foreground)" }}>{'("user_id") '}</span>
                <span style={{ color: "var(--primary)" }}>{"REFERENCES "}</span>
                <span style={{ color: "var(--foreground)" }}>{'"users" ("id");'}</span>
              </code>
            </pre>
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 md:py-24 px-4 sm:px-6">
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-4 sm:mb-6">
            Ready to design your <span style={{ color: "var(--primary)" }}>schema?</span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg mb-8 sm:mb-10" style={{ color: "var(--muted-foreground)" }}>
            Start for free. No account required. Just open the app and start
            designing.
          </p>
          <Link
            to="/app"
      ...
      {/* Footer */}
      <footer className="py-6 sm:py-8 px-4 sm:px-6 border-t text-center text-xs sm:text-sm" style={{ borderColor: "var(--border)", color: "var(--muted-foreground)" }}>
        <div className="flex items-center justify-center gap-2 mb-2">
          <div className="w-4 h-4 sm:w-5 sm:h-5 rounded flex items-center justify-center" style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>
            <span className="font-black text-[9px] sm:text-[10px]">E</span>
          </div>
          <span className="font-semibold" style={{ color: "var(--foreground)" }}>Ember</span>
        </div>
